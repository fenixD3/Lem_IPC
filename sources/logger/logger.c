...
void create_log_file(t_files **files_info)
{
	...
	sprintf(new_file_info->file_name, "%s%s%s", LOG_PATH, ctime(&curr_time), ".txt");
	new_file_info->available_space = (size_t *)get_shm(LOG_SHM_SIZE_NAME, sizeof(size_t));
	new_file_info->file_mapped = get_shm(new_file_info->file_name, LOG_FILE_SIZE);
	//if ((file_fd = open(new_file_info->file_name, O_CREAT | O_EXCL | O_RDWR, 0644)) == -1)
	//{
	//	if (errno == EEXIST)
	//	{
	//		if ((file_fd = open(new_file_info->file_name, O_CREAT | O_RDWR, 0644)) == -1)
	//		{
	//			perror("open_log");
	//			exit(EXIT_FAILURE);
	//		}
...
void destroy_logger(t_logger *logger)
{
	...
	while(curr_file)
	{
		is_error = destroy_shm((void **)&curr_file->available_space, LOG_SHM_SIZE_NAME, sizeof(size_t));
